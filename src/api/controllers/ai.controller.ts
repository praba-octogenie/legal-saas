...
export class AIController {
   ...
   */
  static async askLegalQuestion(req: Request, res: Response): Promise<void> {
    try {
      // Validate request
      if (!req.body) {
      ...
      const citations: { text: string; source: string; url?: string }[] = [];
      const relevantStatutes: { name: string; section: string; text: string }[] = [];
      ...
      logger.error('Error in askLegalQuestion:', error);
      res.status(500).json({
   ...
   */
  static async analyzeLegalDocument(req: Request, res: Response): Promise<void> {
    try {
      // Validate request
      if (!req.body) {
        res.status(400).json({ error: 'Invalid request data' });
      ...
      const risks = risksText
        .split(/\n\s*[-•*]\s*/)
        .filter((risk: string) => risk.trim().length > 0)
      ...
      const obligations = obligationsText
        .split(/\n\s*[-•*]\s*/)
        .filter((obligation: string) => obligation.trim().length > 0)
        .map((obligation: string) => obligation.trim());
      ...
      const recommendations = recommendationsText
        ...
        .map((recommendation: string) => recommendation.trim());
      ...
      logger.error('Error in analyzeLegalDocument:', error);
      res.status(500).json({
   ...
   */
  static async predictCaseOutcome(req: Request, res: Response): Promise<void> {
    try {
      // Validate request
      if (!req.body) {
        res.status(400).json({ error: 'Invalid request data' });
      ...
      const favorable = favorableText
        .split(/\n\s*[-•*]\s*/)
        .filter((factor: string) => factor.trim().length > 0)
      ...
      logger.error('Error in predictCaseOutcome:', error);
      res.status(500).json({
